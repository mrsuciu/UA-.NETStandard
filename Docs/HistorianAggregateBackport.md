...
`master` into `master378-CTTBackport`. It deliberately keeps the
...
`Tests/Opc.Ua.Server.Tests/HistorianAggregateBackportManifest.json`. It maps all
356 source tests found in the 22 historian/aggregate source files to one of:
exact, API-adapted, fixture-adapted, or framework-only exclusion. Every entry
records its source commit, destination coverage, direction, bounds,
configuration, expected values/timestamps/statuses, and any deliberate
